"""Background-loader lifecycle: health transitions and /parse availability.

These cover the path where ``create_app`` is built WITHOUT an injected pipeline,
so it loads one in a background thread. No GPU/torch needed: ``_load_pipeline``
imports ``omni_server.inference`` lazily, so we inject a fake into ``sys.modules``
and drive the same loader code path to reach the loading -> ready / error states.
"""
...
def test_health_reports_loading_before_pipeline_ready() -> None:
    ...
    client = TestClient(create_app(Settings(warmup=False)))
...
def test_health_reports_ok_once_pipeline_ready(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(create_app(Settings(warmup=True))) as client:
...
def test_health_reports_error_on_load_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(create_app(Settings(warmup=False))) as client:
